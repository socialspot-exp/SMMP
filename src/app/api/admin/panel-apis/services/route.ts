...
/**
 * Perfect Panel–style services list: POST form key + action=services.
 * Returns normalized `{ services: { id, name, category, type, rate, min, max }[] }` when JSON is an array.
 */
export async function POST(request: Request) {
  ...
  if (Array.isArray(payload)) {
    for (const item of payload) {
      ...
      const o = item as RawSvc;
      const sid = o.service ?? o.id;
      if (sid == null || sid === "") continue;
      const idStr = String(sid);
      const name = typeof o.name === "string" ? o.name : idStr;
      services.push({
        id: idStr,
        name,
        ...
        min: o.min != null ? String(o.min) : null,
        max: o.max != null ? String(o.max) : null,
      });
    }
  }
